...
import { canonicalObjectTitle, canonicalizeObjectIdentities, rewriteObjectReferences } from './object-identity.js';
import { isEntryLifecycleHidden, normalizeEntryLifecycleValue, visibleStateRows, } from './entry-lifecycle.js';
import { enforceSpacetimeSingleton, isHistoricalSceneRow } from './special-table-rules.js';
import { isPurePassiveObserverText } from './observer.js';
import { dedupeStrongStateRows, mergeDuplicateStateRows } from './state-text.js';
import { DEFAULT_TABLE_REGISTRY, migrateSnapshotTables, normalizeTableRegistry, tableByKey, tableByRole } from './table-registry.js';
export { rewriteObjectReferences } from './object-identity.js';
// 生命周期写操作只允许从 entry-lifecycle / memory-state-machine 进入。
// snapshot 仅保留只读参与状态辅助函数，避免形成第二个写入入口。
export { isEntryLifecycleHidden, isEntryParticipationPaused, visibleStateRows, } from './entry-lifecycle.js';
