...
const __ma_module_2 = (() => {
  ...
  return Object.freeze({ WORLD_SCHEMA, WORLD_TYPES, EXTRACTION_TYPES, hasType, hasSection, sectionNames, sectionPolicy, renderSchema, FACT_PROTOCOL, KEYWORD_PROTOCOL, SUMMARY_PROTOCOL });
})();
...
const __ma_module_10 = (() => {
  const { EXTRACTION_TYPES, FACT_PROTOCOL, KEYWORD_PROTOCOL, SUMMARY_PROTOCOL, WORLD_TYPES, hasSection, hasType, sectionNames } = __ma_module_2;
...
const __ma_module_13 = (() => {
  const { EXTRACTION_TYPES, FACT_PROTOCOL, KEYWORD_PROTOCOL, SUMMARY_PROTOCOL, WORLD_TYPES, renderSchema } = __ma_module_2;
  ...
  function importPrompt(sourceText, relevantEntries) {
    ...
    return {
      system: `把玩家主动提交的世界设定整理为世界书事实。模型只决定语义类型、稳定名称、栏目、主要关键词与事实；插件决定UID、唯一身份和写入。不得把写作要求、提示规则、未来计划或未成立事件写成世界事实。世界长期通用规则、底层规律和不随剧情自然变化的设定归入“基础设定”；具体人物、场景、物品、事件和可变化的世界状态仍按对应类型整理。所有本次导入条目写入后由插件设置为基石只读。\n\n允许类型与栏目：\n${renderSchema(WORLD_TYPES)}\n${schemaFieldDiscipline}\n\n${keywordDiscipline}\n每条事实严格使用：\n${FACT_PROTOCOL}\n本任务的变化字段统一写“建立”。同一“类型＋稳定名称”的多行会形成同一个候选条目。没有可导入事实时只输出“无”。本批最多32个候选对象。\n${outputDiscipline}`,
    ...
    async extract(settings, snapshot, progress = () => undefined) {
      ...
      const response = await this.model.structured(
        ...
        raw => validateFactRelationTokens(
          parseFactProtocol(raw, { allowedTypes: EXTRACTION_TYPES, maxIdentities: 32, requireKeywords: true }),
          opened.managed,
        ),
      );
      this.host.assertSnapshot(snapshot);
...
const __ma_module_18 = (() => {
    ...
    refresh() {
      ...
      try {
        ...
        messages.forEach((message, order) => {
          ...
          if (latest) appendLatestText(indicator, state, messageState, settings);
  ...
  function summaryReminderText(state = {}, settings = {}) {
    ...
    if (settings.autoSmallSummary !== false) {
      if (closed) small = `小总结：待处理${closed}场`;
      else small = settings.autoExtraction === false ? '小总结：等待自动提取' : '小总结：换场景后';
    }
    let large = '大总结：手动';
    if (settings.autoLargeSummary !== false) {
      large = completed > batch ? `大总结：待处理最早${batch}组` : `大总结：第${batch + 1}组触发（${completed}/${batch + 1}）`;
    }
    return `${small}｜${large}`;
  }
  ...
  function appendLatestText(indicator, state, messageState, settings) {
    const main = document.createElement('span');
    main.className = 'ma-chat-indicator-main';
    main.textContent = latestText(state, messageState);
    const reminder = document.createElement('span');
    reminder.className = 'ma-chat-summary-reminder';
    reminder.textContent = summaryReminderText(state, settings);
...
const __ma_module_21 = (() => {
  ...
  function entrySettings(controller, entry) {
    ...
    const beforeDefinition = insertionButton(controller, entry, insertionPosition, 0, readonly);
    const afterDefinition = insertionButton(controller, entry, insertionPosition, 1, readonly);
    ...
    actions.push(save);
    const cornerstone = taskButton(readonly ? '解除基石' : '设为基石', async () => {
      await controller.setCornerstone([entry.uid], !readonly);
      inform(readonly ? '已解除基石锁' : '已设为基石只读');
    }, readonly ? 'is-active' : '', readonly ? '解除中…' : '设置中…');
    cornerstone.setAttribute('aria-pressed', String(readonly));
    actions.push(cornerstone);
    ...
    return element('section', { className: `ma-entry-settings${readonly ? ' is-readonly' : ''}` }, [
      ...
      element('div', { className: 'ma-insertion-settings' }, [
        element('strong', { text: '角色定义位置' }), beforeDefinition, afterDefinition,
      ]),
      ...
      (entry.secondaryKeywords ?? []).length > 0 && entry.selective !== true
        ...
        : null,
      readonly ? element('p', { className: 'ma-detail-note', text: '基石条目只读；解除基石后可修改召回、主焦点与插入位置。' }) : null,
      element('div', { className: 'ma-entry-settings-actions' }, actions),
    ]);
  }
  
  function insertionButton(controller, entry, current, position, readonly) {
    const label = position === 0 ? '定义前' : '定义后';
    const control = taskButton(label, async () => {
      if (current === position) return;
      await controller.setInsertionPosition([entry.uid], position);
      inform(`已移到角色定义${position === 0 ? '前' : '后'}`);
    }, current === position ? 'is-active' : '', '保存中…');
...
const __ma_module_24 = (() => {
  ...
  const { notesManageBar } = __ma_module_23;
  class NotesPage {
    constructor(panel) {
      this.panel = panel;
    ...
    detailActions(worldbookName, entry, folders, layout, body) {
      const toolbar = entry.cornerstone ? null : element('div', { className: 'ma-detail-toolbar' }, [
...
const __ma_module_26 = (() => {
  ...
  const { NotesPage } = __ma_module_24;
  const { RunPage } = __ma_module_25;
...
const __ma_module_27 = (() => {
  ...
  const { MirrorAbyssPanel } = __ma_module_26;
...
const { createApplication } = __ma_module_27;
