...
function normalizedRole(value2) {
  const allowed = /* @__PURE__ */ new Set(["spacetime", "scenes", "characters", "items", "events", "regions", "globalChanges", "foundations", "custom", "focus", "state", "skills", "relationships"]);
...
function migrateSnapshotTables(value2, registry2) {
  ...
  const distribute = (rows, field, objectType) => {
    ...
    for (const raw of rows) {
      const row = deepClone(raw);
      const text2 = normalizedName(rowText(row));
      const matches = characterNames.filter((item) => item.names.some((name) => text2.includes(name)));
...
function getSettings() {
  ...
  settings.statePrompts.admissionRules = safeText(settings.statePrompts.admissionRules, 8e3).trim();
  settings.statePrompts.exclusionRules = safeText(settings.statePrompts.exclusionRules, 8e3).trim();
  settings.statePrompts.routingRules = safeText(settings.statePrompts.routingRules, 8e3).trim();
  settings.statePrompts.evidenceRules = safeText(settings.statePrompts.evidenceRules, 8e3).trim();
  settings.statePrompts.updateRules = safeText(settings.statePrompts.updateRules, 8e3).trim();
  ...
  for (const [key, fallback] of Object.entries(DEFAULT_CONTENT_LIMITS.tables)) {
    const current2 = Number(settings.contentLimits.tables[key]);
    settings.contentLimits.tables[key] = Math.min(2e4, Math.max(200, Math.round(current2 || fallback)));
  }
  for (const table of settings.tableRegistry ?? []) {
    ...
    const current2 = Number(settings.contentLimits.tables[table.key]);
    settings.contentLimits.tables[table.key] = Math.min(2e4, Math.max(200, Math.round(current2 || fallback)));
  }
  ...
  if (!settings.migration.entryRoutingV33) {
    const appendRule = (current2, rule, marker) => current2.includes(marker) ? current2 : `${current2.trim()}${current2.trim() ? "\n" : ""}${rule}`;
    settings.statePrompts.admissionRules = appendRule(
      settings.statePrompts.admissionRules,
      "\u7269\u54C1\u53EA\u8981\u5176\u8EAB\u4EFD\u53EF\u533A\u5206\uFF0C\u4E14\u6240\u6709\u6743\u3001\u4F4D\u7F6E\u3001\u6570\u91CF\u3001\u5B8C\u6574\u6027\u3001\u53EF\u7528\u6027\u3001\u9690\u85CF\u72B6\u6001\u3001\u7528\u9014\u6216\u540E\u7EED\u80FD\u5426\u53D6\u5F97\u4F1A\u5F71\u54CD\u4E0B\u4E00\u8F6E\uFF0C\u5C31\u5E94\u5EFA\u6863\uFF1B\u4E0D\u8981\u6C42\u5DF2\u7ECF\u591A\u6B21\u51FA\u73B0\u3002",
      "\u540E\u7EED\u80FD\u5426\u53D6\u5F97"
    );
    settings.statePrompts.admissionRules = appendRule(
      settings.statePrompts.admissionRules,
      "\u5F53\u524D\u5B9E\u9645\u53D1\u751F\u7684\u53D9\u4E8B\u573A\u666F\u5FC5\u987B\u5EFA\u7ACB\u6216\u66F4\u65B0\u573A\u666F\u6761\u76EE\uFF1B\u573A\u666F\u6761\u76EE\u8BB0\u5F55\u53C2\u4E0E\u5BF9\u8C61\u3001\u6838\u5FC3\u5C40\u9762\u3001\u76F4\u63A5\u9650\u5236\u4E0E\u672A\u51B3\u627F\u63A5\uFF0C\u4E0D\u7B49\u540C\u4E8E\u5730\u70B9\u672C\u8EAB\u3002",
      "\u4E0D\u7B49\u540C\u4E8E\u5730\u70B9\u672C\u8EAB"
    );
    settings.statePrompts.routingRules = appendRule(
      settings.statePrompts.routingRules,
      "scenes\uFF08\u573A\u666F\uFF09\uFF1A\u4E00\u6B21\u771F\u5B9E\u53D1\u751F\u7684\u53D9\u4E8B\u573A\u666F\u6216\u5C40\u9762\u5207\u7247\uFF1B\u573A\u666F\u7ED3\u675F\u540E\u6539\u4E3A\u5DF2\u7ED3\u675F/\u5DF2\u79BB\u5F00\uFF0C\u4E0D\u5220\u9664\u3002",
      "scenes\uFF08\u573A\u666F\uFF09"
    );
    settings.statePrompts.routingRules = appendRule(
      settings.statePrompts.routingRules,
      "\u5206\u7C7B\u65F6\u5148\u5224\u65AD\u5BF9\u8C61\u672C\u8D28\uFF1A\u5177\u4F53\u4E2A\u4F53\u624D\u662F\u89D2\u8272\uFF1B\u7EC4\u7EC7\u3001\u9635\u8425\u3001\u653F\u6743\u3001\u673A\u6784\u3001\u7FA4\u4F53\u683C\u5C40\u548C\u5236\u5EA6\u6267\u884C\u5373\u4F7F\u5177\u6709\u540D\u79F0\u3001\u76EE\u6807\u3001\u5173\u7CFB\u6216\u884C\u52A8\uFF0C\u4E5F\u5F52\u5165\u5168\u5C40\u53D8\u5316\u3002",
      "\u5177\u4F53\u4E2A\u4F53\u624D\u662F\u89D2\u8272"
    );
    settings.statePrompts.updateRules = appendRule(
      settings.statePrompts.updateRules,
      "\u63D0\u4EA4\u524D\u9010\u9879\u590D\u67E5\u6B63\u6587\u4E2D\u7684\u53EF\u533A\u5206\u7269\u54C1\uFF0C\u51E1\u6240\u6709\u6743\u3001\u4F4D\u7F6E\u3001\u6570\u91CF\u3001\u5B8C\u6574\u6027\u3001\u53EF\u7528\u6027\u3001\u9690\u85CF\u72B6\u6001\u3001\u7528\u9014\u6216\u540E\u7EED\u53EF\u53D6\u5F97\u6027\u88AB\u5EFA\u7ACB\u6216\u6539\u53D8\uFF0C\u5747\u9700\u8F93\u51FA\u72EC\u7ACB\u7269\u54C1\u6761\u76EE\u3002",
      "\u9010\u9879\u590D\u67E5\u6B63\u6587\u4E2D\u7684\u53EF\u533A\u5206\u7269\u54C1"
    );
    settings.statePrompts.updateRules = appendRule(
      settings.statePrompts.updateRules,
      "\u63D0\u4EA4\u524D\u786E\u8BA4\u5F53\u524D\u771F\u5B9E\u573A\u666F\u6709\u573A\u666F\u6761\u76EE\uFF1B\u53D1\u751F\u573A\u666F\u5207\u6362\u65F6\uFF0C\u65B0\u573A\u666F\u8BBE\u4E3A\u5F53\u524D\uFF0C\u65E7\u573A\u666F\u66F4\u65B0\u4E3A\u5DF2\u7ED3\u675F\u6216\u5DF2\u79BB\u5F00\u3002",
      "\u5F53\u524D\u771F\u5B9E\u573A\u666F\u6709\u573A\u666F\u6761\u76EE"
    );
...
function eventIdFor(source, factId2) {
  ...
  return `event_${hashText(`${entity}|${source.title}|${factId2}`)}`;
}
function normalizeInternalFact(value2, sourceMessageId = "", index = 0) {
  ...
  const factId2 = explicitFactId || `fact_${hashText(`${title}|${content}|${index}`)}`;
  ...
  return {
    factId: factId2,
    eventId: eventIdFor(source, factId2),
...
function rowKind(value2) {
  return KIND_ALIASES[identity(value2)];
}
function semanticTable(kind, active) {
  return kind ? tableByRole(active, KIND_ROLE[kind], false) : void 0;
}
...
var STANDARD_FIELDS = /* @__PURE__ */ new Set(["id", "title", "name", "content", "summary", "keywords", "status", "source", "locked", "lockMode", "lifecycle", "entryLifecycle", "updatedAt", "factIds", "fact_ids", "eventId", "event_id", "eventIds", "event_ids", "recall", "fields"]);
...
function enumValue(value2, allowed, fallback) {
  const text2 = safeText(value2, 80).trim();
  return allowed.has(text2) ? text2 : fallback;
}
...
var LEGACY_CHARACTER_STATE_FIELDS = ["currentFacts", "currentStates", "recentHistory", "relationshipStates", "abilityStates", "presentationStates"];
...
function enforceObjectViewAllocation(snapshot, registry2) {
  ...
  snapshot[regionKey] = (snapshot[regionKey] ?? []).filter((row) => {
    ...
    const text2 = regionStateText(row);
    if (hasHistory || persistentChangeSignal.test(text2)) return true;
...
function ensureCurrentSceneEntry(snapshot, registry2) {
  ...
  if (existing) {
    ...
    existing.keywords = [.../* @__PURE__ */ new Set([...existing.keywords ?? [], currentSpacetime.title, ...currentSpacetime.keywords ?? []])];
...
function snapshotRowCount(snapshot, registry2, enabledOnly = false) {
  ...
  return tables2.reduce((sum, table) => sum + visibleStateRows(snapshot[table.key]).length, 0);
}
...
function moveManualRow(snapshot, sourceTableKey, targetTableKey, rowId, row, registry2) {
  const tables2 = registryOrDefault(registry2);
  if (sourceTableKey === targetTableKey) return upsertManualRow(snapshot, sourceTableKey, { ...row, id: rowId }, tables2);
  const next = normalizeSnapshot(snapshot, snapshot, tables2);
  const source = (next[sourceTableKey] ?? []).find((item) => item.id === rowId);
  if (!source) throw new Error("\u8981\u79FB\u52A8\u7684\u6761\u76EE\u4E0D\u5B58\u5728\u6216\u5DF2\u88AB\u66F4\u65B0");
  ...
  if (targetId !== previousId) moved = rewriteObjectReferences(moved, /* @__PURE__ */ new Map([[previousId, targetId]]));
  return dedupeStrongStateRows(moved, tables2);
}
  ...
  for (const message of getChat()) {
    ...
    if ((needsFullSnapshotMigration || needsPersistedCharacterMergeMigration || needsUniqueObjectNamesMigration) && artifact.snapshot) {
      ...
      if (needsUniqueObjectNamesMigration) {
        ...
        const unique3 = mergeDuplicateStateRows(migrated, registry2);
        migrated = unique3.snapshot;
        const artifactAliases = new Map(savedAliases);
        for (const [oldId, stableId] of unique3.idRemap) artifactAliases.set(oldId, stableId);
        ...
        if (unique3.mergedCount || crossSnapshotAliases.size || savedAliases.size) {
          uniqueObjectNamesChanged = true;
          if (artifact.stages?.sync) {
            artifact.stages.sync = {
              ...artifact.stages.sync,
...
function textFromValue(value2) {
  ...
  for (const key of ["text", "output_text", "content", "value"]) {
    ...
    if (Array.isArray(nested)) {
      const text2 = textFromContentParts(nested);
...
function textFromContentParts(value2) {
  ...
  for (const item of value2) {
    ...
    const source = item;
    const text2 = typeof source.text === "string" ? source.text.trim() : typeof source.output_text === "string" ? source.output_text.trim() : "";
...
function generationText(result) {
  ...
  for (const value2 of [result.output_text, result.content, result.text, result.result, result.value, result.pipe]) {
    if (Array.isArray(value2)) {
      const text3 = textFromContentParts(value2);
      if (text3) return text3;
      continue;
    }
    const text2 = textFromValue(value2);
    if (text2) return text2;
  }
  const messageContent = result?.message?.content ?? result?.choices?.[0]?.message?.content;
  if (Array.isArray(messageContent)) {
    const text2 = textFromContentParts(messageContent);
    if (text2) return text2;
  } else {
    const text2 = textFromValue(messageContent);
    if (text2) return text2;
  }
  for (const value2 of [
...
async function generateCurrent(options, controller) {
  ...
  const text2 = generationText(result);
  if (!text2) throw emptyGenerationError(`${options.task}\u6A21\u578B`, result);
  return text2;
}
async function generateWithNativeProfile(options, profileId, controller) {
  ...
  const text2 = generationText(result);
  if (!text2) throw emptyGenerationError(`${options.task} Connection Profile`, result);
  return text2;
}
...
function parseAuditTextOutput(raw) {
  const text2 = safeText(raw, 22e4).trim();
  const legacy = text2.replace(/\r/g, "").split("\n");
  ...
  const blocks = parseFixedTextBlocks(text2, MARKERS);
...
function alignRevisionStageWithAudit(artifact, result) {
  ...
  const committedRevision = Boolean(
    currentRevision?.status === "success" && currentRevision.finalFingerprint && currentRevision.finalFingerprint === artifact.sourceFingerprint
  );
  if (result.passed) {
    if (!committedRevision) markStage(artifact, "revision", "skipped");
    return;
  }
...
${settings.maxRevisionAttempts}`);
  ...
  const cachedTerminalMatches = Boolean(
    cachedAudit && (cachedAudit.passed && artifact.stages.audit.status === "success" && artifact.approvedFingerprint === artifact.sourceFingerprint || !cachedAudit.passed && artifact.stages.audit.status === "blocked")
  );
...
function updateActiveSwipe(message, text2) {
  if (!Array.isArray(message?.swipes)) return;
  const id = Number(message.swipe_id);
  if (Number.isInteger(id) && id >= 0 && id < message.swipes.length) message.swipes[id] = text2;
}
...
async function replaceMessageInPlace(artifact, text2) {
  assertArtifactCommitCurrent(artifact);
  const message = getMessage(artifact.messageIndex);
  if (!message || message.is_user) throw new Error("\u539FAI\u6D88\u606F\u5DF2\u4E0D\u5B58\u5728");
  const finalText = String(text2 || "").trim();
...
function cleanRevisionText(raw) {
  let text2 = safeText(raw, 2e5).trim();
  const fenced = text2.match(/^```(?:markdown|text)?\s*([\s\S]*?)```$/i);
  if (fenced) text2 = fenced[1].trim();
  text2 = text2.replace(/^(?:【?修正版(?:正文)?】?|修正后的完整正文)\s*[:：]?\s*/i, "").trim();
  return text2;
}
...
function boundedLine(label, value2) {
  if (Array.isArray(value2)) {
    return value2.map((item) => String(item ?? "").trim()).filter(Boolean).map((item) => `${label}\uFF1A${item}`);
  }
  const text2 = String(value2 ?? "").trim();
  return text2 ? [`${label}\uFF1A${text2}`] : [];
}
function fitWholeLines(lines2, maxChars) {
  ...
  for (const rawLine of lines2) {
    ...
    if (!output.length) return line.slice(0, Math.max(1, limit - 1)) + (line.length > limit ? "\u2026" : "");
  }
  return output.join("\n");
}
function rowContent(table, row, maxChars) {
  ...
  for (const field of table.fields) {
    if (prioritizedKeys.includes(field.key) || !(field.key in fields)) continue;
    lines2.push(...boundedLine(field.label, fields[field.key]));
  }
  lines2.push(...lifecycleLines(row.lifecycle));
  ...
  return fitWholeLines(lines2, maxChars);
}
...
function filterSnapshotForLorebook(snapshot, customRegistry) {
  ...
  for (const table of enabledTables(tables2)) {
    if (["focus", "spacetime", "scenes", "characters", "state", "items", "events", "globalChanges", "foundations"].includes(table.role)) continue;
    next[table.key] = (next[table.key] ?? []).filter((row) => {
      if (table.role === "custom" && safeText(row.fields?.migrationStatus, 80).trim() === "\u5DF2\u5F52\u5E76") return false;
      if (row.source === "manual" || row.locked) return true;
      if (isAudienceRow(row)) return false;
      const text2 = normalizedName2(rowSearchText(row));
      return !retainedNames.size || [...retainedNames].some((name) => text2.includes(name)) || table.role === "regions";
    });
  }
  return next;
}
...
function recallModeFor(role, row, options, currentSpacetimeId) {
  ...
  if (role === "scenes") return row.id === currentSpacetimeId ? "constant" : "trigger";
...
function tableDocuments(snapshot, options) {
  ...
  for (const table of enabledTables(tables2)) {
    ...
    const currentSpacetimeId = table.role === "spacetime" || table.role === "scenes" ? currentSpacetimeRowId(rows) : void 0;
    for (const row of rows) {
      ...
      docs.push(makeDocument(
        ...
        rowContent(table, row, Math.max(200, Number(options.entryLimits?.[table.key]) || Number(DEFAULT_CONTENT_LIMITS.tables[table.key]) || 1200)),
        `view:${table.role}`,
        mode,
        trigger,
        row.factIds ?? [],
        ...
        row.updatedAt,
        options,
        isEntryParticipationPaused(row)
      ));
    }
  }
  return docs;
...
title=\u4E8B\u4EF6\u7EBF\u6807\u9898
summary=${kind === "small" ? "\u5F53\u524D\u552F\u4E00\u6709\u6548\u7684\u8FD1\u671F\u4E8B\u5B9E\u603B\u7ED3" : "\u5F53\u524D\u552F\u4E00\u6709\u6548\u7684\u957F\u671F\u4E8B\u5B9E\u603B\u7ED3"}
keyword=\u5173\u952E\u8BCD\uFF08\u53EF\u91CD\u590D\u591A\u884C\uFF09
</MA_SUMMARY>`;
}
...
6. summary \u6B63\u6587\u4E0D\u5F97\u8D85\u8FC7 ${Math.max(200, Math.round(maxChars))} \u4E2A\u5B57\u7B26\uFF1B\u53EA\u5220\u9664\u91CD\u590D\u3001\u5DF2\u8986\u76D6\u8FC7\u7A0B\u4E0E\u65E0\u4E8B\u5B9E\u5185\u5BB9\uFF0C\u4E0D\u5F97\u65B0\u589E\u4E8B\u5B9E\u3002
7. \u4E0D\u8F93\u51FA unresolved\u3001\u5EFA\u8BAE\u3001\u8BC4\u4EF7\u3001\u6F5C\u5728\u4EF7\u503C\u6216\u5904\u7406\u65B9\u6848\u3002

\u56FA\u5B9A\u8F93\u51FA\u534F\u8BAE\uFF1A
${summaryFixedProtocol(kind)}`;
}
function smallSummarySystemPrompt(sections = DEFAULT_SUMMARY_PROMPTS.small, maxChars = 700) {
...
function largeSummarySystemPrompt(sections = DEFAULT_SUMMARY_PROMPTS.large, maxChars = 1800) {
...
async function generateSmallSummary(artifact, force = false) {
  ...
  const generated = slotted.map((group) => {
    ...
    const text2 = bySlot.get(group.slot);
    const summary = normalizeSummary({ title: text2.title, summary: text2.summary, keywords: text2.keywords, unresolved: text2.unresolved }, "small", sourceFactIds, void 0, { eventId: group.eventId, sourceFactIds });
    ...
    if (summary.summary.length > settings.contentLimits.smallSummary) throw new Error(`\u5C0F\u603B\u7ED3\u8D85\u8FC7\u767D\u76D2\u786C\u4E0A\u9650\uFF1A${summary.summary.length}/${settings.contentLimits.smallSummary} \u5B57\uFF08${group.eventId}\uFF09`);
...
async function generateLargeSummary(artifact, force = false) {
  ...
  const generated = slotted.map((group) => {
    const text2 = bySlot.get(group.slot);
    const value2 = { title: text2.title, summary: text2.summary, keywords: text2.keywords, unresolved: text2.unresolved };
    ...
    if (summary.summary.length > settings.contentLimits.largeSummary) throw new Error(`\u5927\u603B\u7ED3\u8D85\u8FC7\u767D\u76D2\u786C\u4E0A\u9650\uFF1A${summary.summary.length}/${settings.contentLimits.largeSummary} \u5B57\uFF08${group.eventId}\uFF09`);
...
function filterPassiveObserverFacts(facts) {
  return facts.filter((fact) => {
    ...
    return !isPurePassiveObserverText(text2);
  });
}
...
function pushContextField(lines2, key, value2) {
  ...
  const text2 = contextValue(value2);
  if (text2) lines2.push(`${key}=${text2}`);
}
...
function assertStateBusinessShape(parsed, active) {
  ...
  parsed.facts.forEach((fact, index) => {
    ...
    const source = fact;
    const factId2 = String(source.fact_id ?? source.factId ?? source.id ?? "").trim();
    const eventId = String(source.event_id ?? source.eventId ?? source.entity_id ?? source.entityId ?? "").trim();
    const title = String(source.title ?? "").trim();
    if (!factId2) throw new Error(`facts[${index}].fact_id \u4E0D\u80FD\u4E3A\u7A7A`);
...
function applyStateRowRelocations(previous, rawRelocations, registry2) {
  if (!Array.isArray(rawRelocations) || !rawRelocations.length) return previous;
  const next = structuredClone(previous);
  const tableKeys = new Set(registry2.map((table) => table.key));
  for (const raw of rawRelocations) {
    ...
    if (!id || !fromTable || !toTable || fromTable === toTable) continue;
    if (!tableKeys.has(fromTable) || !tableKeys.has(toTable)) continue;
    const sourceRows = next[fromTable] ?? [];
    const source = sourceRows.find((row) => row.id === id);
    if (!source) continue;
    if (source.source === "manual" || source.locked || source.lockMode === "all" || source.lockMode === "base") continue;
    next[fromTable] = sourceRows.filter((row) => row.id !== id);
  }
  return normalizeSnapshot(next, previous, registry2);
}
...
async function processMessage(index, force = false, options = {}) {
  ...
  const identity3 = messageIdentity(index);
  ...
  const key = `${PIPELINE_VERSION}:${scheduledChatKey}:${identity3}`;
  const attachedAtEnqueue = getAttachedArtifact(message);
  const duplicateCommittedAutomatic = Boolean(
    options.automatic && !force && !options.historyRecovery && attachedAtEnqueue && attachedAtEnqueue.chatKey === scheduledChatKey && attachedAtEnqueue.sourceFingerprint === scheduledFingerprint && attachedAtEnqueue.messageKey === identity3 && attachedAtEnqueue.snapshot && attachedAtEnqueue.stages.state.status === "success" && enqueueState.processedMessageKeys.includes(attachedAtEnqueue.messageKey)
  );
  ...
  return taskQueue.run(key, `\u5904\u7406\u7B2C ${index + 1} \u6761AI\u6B63\u6587`, "state", async (guard) => {
    ...
    const alreadyCommitted = Boolean(
      options.automatic && !force && !options.historyRecovery && attached && attached.chatKey === scheduledChatKey && attached.sourceFingerprint === scheduledFingerprint && attached.messageKey === identity3 && attached.snapshot && attached.stages.state.status === "success" && processingState.processedMessageKeys.includes(attached.messageKey)
    );
    if (alreadyCommitted) {
...
async function retryStage(index, stage) {
  ...
  const identity3 = messageIdentity(index);
  const key = `${PIPELINE_VERSION}:retry:${stage}:${chatKey}:${identity3}`;
...
// src/domain/event-profile.ts
function unique2(values2, limit = 80) {
  return [...new Set(values2.map((value2) => String(value2 || "").trim()).filter(Boolean))].slice(0, limit);
}
...
function buildEventProfiles(snapshot, facts, smallSummaries, largeSummaries, registry2) {
  const tables2 = normalizeTableRegistry(registry2);
  const eventTable = tableByRole(tables2, "events", false);
  const eventRows = eventTable ? snapshot?.[eventTable.key] ?? [] : [];
  const eventRowById = /* @__PURE__ */ new Map();
  for (const row of eventRows) {
    eventRowById.set(row.id, row);
    for (const eventId of eventIdsOf(row)) eventRowById.set(eventId, row);
  }
  const ids = /* @__PURE__ */ new Set();
  ...
  ids.delete("");
  const enabled = enabledTables(tables2);
  const profiles = [];
  for (const eventId of ids) {
    ...
    const relatedEntities = unique2([
      ...eventFacts.flatMap((fact) => fact.relatedEntities ?? []),
      ...relatedEntries.map((entry) => entry.title)
    ], 24);
    const currentResults = unique2(eventFacts.flatMap((fact) => fact.occurredFacts ?? []).filter(Boolean), 8);
    const allFactsClosed = eventFacts.length > 0 && eventFacts.every(factClosed2);
    ...
    profiles.push({
      ...
      contentChars: eventFacts.reduce((sum, fact) => sum + (fact.occurredFacts ?? []).join("").length + String(fact.content || "").length, 0) + small.reduce((sum, summary) => sum + summary.summary.length, 0) + large.reduce((sum, summary) => sum + summary.summary.length, 0),
      updatedAt,
      currentResults
    });
  }
...
function redactedError(value2) {
  ...
  return text2.replace(/([；;]\s*(?:原始)?返回片段\s*[:：]).*$/s, "$1[\u5DF2\u9690\u85CF]").slice(0, 1200);
}
...
var diagnosticPromptKind = "state";
...
function promptSettingsAreStandard() {
  const settings = getSettings();
  return JSON.stringify(settings.statePrompts) === JSON.stringify(DEFAULT_STATE_PROMPTS) && JSON.stringify(settings.summaryPrompts) === JSON.stringify(DEFAULT_SUMMARY_PROMPTS);
}
...
function searchableRowText(row) {
  return [
    ...
    row.status,
    ...row.keywords ?? [],
    ...Object.values(row.fields ?? {}).flatMap((value2) => Array.isArray(value2) ? value2.map(String) : [String(value2 ?? "")])
...
async function tableHtml(artifactInfo) {
  ...
  return `
    ...
    <p class="ma11-table-hint">\u53EA\u663E\u793A\u542F\u7528\u89C6\u56FE\u3002\u624B\u673A\u7AEF\u53EF\u5728\u8868\u683C\u533A\u57DF\u5DE6\u53F3\u6ED1\u52A8\uFF1B\u624B\u673A\u7AEF\u4E0D\u56FA\u5B9A\u524D\u4E24\u5217\uFF0C\u907F\u514D\u906E\u6321\u7F16\u8F91\u5185\u5BB9\u3002</p>
...
function statePromptEditorHtml(registry2) {
  const settings = getSettings();
...
function tableManagerHtml(artifactInfo) {
  ...
  return `<section class="ma11-toolbar ma11-rules-toolbar"><div><h2>\u8BB0\u5FC6\u89C4\u5219\u4E0E\u8868\u683C</h2><p>\u6309\u201C\u72B6\u6001\u8868 \u2192 \u5C0F\u603B\u7ED3 \u2192 \u5927\u603B\u7ED3 \u2192 \u8868\u683C\u5B57\u6BB5\u201D\u7684\u987A\u5E8F\u914D\u7F6E\uFF1B\u666E\u901A\u4F7F\u7528\u4FDD\u7559\u6807\u51C6\u503C\u5373\u53EF\u3002</p></div><div class="ma11-actions"><button data-ma11-action="restore-standard-rules">\u5E94\u7528\u6807\u51C6\u89C4\u5219</button><button data-ma11-action="open-prompt-diagnostics">\u67E5\u770B\u8C03\u8BD5\u9884\u89C8</button></div></section>
    ...
    ${statePromptEditorHtml(registry2)}
...
function summaryPromptEditorHtml(kind) {
  ...
  const description = kind === "small" ? "\u628A\u540C\u4E00\u4E8B\u4EF6\u7EBF\u6574\u7406\u6210\u4E0B\u4E00\u6B65\u5FC5\u987B\u627F\u63A5\u7684\u6700\u5C0F\u4E8B\u5B9E\u96C6\u5408\uFF0C\u5E76\u5199\u5165\u8868\u683C\u7684\u201C\u8FD1\u671F\u7ECF\u5386\u201D\u3002" : "\u91CD\u65B0\u5BA1\u67E5\u5C0F\u603B\u7ED3\uFF0C\u53EA\u4FDD\u7559\u8DE8\u573A\u666F\u3001\u8DE8\u9636\u6BB5\u4ECD\u4F1A\u6539\u53D8\u672A\u6765\u7684\u4E8B\u5B9E\uFF0C\u5E76\u5199\u5165\u8868\u683C\u7684\u201C\u5386\u53F2\u4E8B\u5B9E\u201D\u3002";
...
function connectionBlock(task, label) {
  ...
  return `<div class="ma11-connection-row" data-ma11-connection="${task}">
    ...
    <select data-ma11-connection-mode="${task}">
      <option value="current" ${value2.mode === "current" ? "selected" : ""}>\u6CBF\u7528\u5F53\u524D\u804A\u5929\u6A21\u578B</option>
      <option value="profile" ${value2.mode === "profile" ? "selected" : ""}>\u4F7F\u7528\u72EC\u7ACB\u6A21\u578B\u914D\u7F6E</option>
    </select>
...
function openRowEditor(tableKey, row) {
  ...
  targetTable.innerHTML = enabledTables(getSettings().tableRegistry).map((table) => `<option value="${escapeHtml(table.key)}">${escapeHtml(table.name)}</option>`).join("");
...
async function saveRow(form) {
  ...
  const sourceTableKey = form.elements.namedItem("tableKey").value;
  const tableKey = form.elements.namedItem("targetTableKey").value;
  ...
  info.artifact.snapshot = rowId && sourceTableKey !== tableKey ? moveManualRow(info.artifact.snapshot, sourceTableKey, tableKey, rowId, rowPatch, getSettings().tableRegistry) : upsertManualRow(info.artifact.snapshot, tableKey, rowPatch, getSettings().tableRegistry);
  if (rowId) {
...
function applyTableSearch(workspace, value2) {
  tableSearchQuery = value2.trim().toLocaleLowerCase();
  const rows = Array.from(workspace.querySelectorAll("[data-ma11-table-row-search]"));
  let visible = 0;
  for (const row of rows) {
    const matches = !tableSearchQuery || String(row.dataset.ma11TableRowSearch || "").includes(tableSearchQuery);
    row.hidden = !matches;
    if (matches) visible += 1;
  }
  const output = workspace.querySelector("[data-ma11-table-visible-count]");
  if (output) output.value = `${visible} / ${rows.length}`;
}
function applyEventProfileSearch(workspace, value2) {
  graphSearchQuery = value2.trim().toLocaleLowerCase();
  ...
  if (output) output.value = `${visible} / ${cards.length}`;
}
function applyGraphSearch(workspace, value2) {
  graphSearchQuery = value2.trim().toLocaleLowerCase();
  ...
  for (const node of nodes) {
    ...
    if (matches && node.dataset.ma11GraphNode) matched.add(node.dataset.ma11GraphNode);
  }
  ...
  if (output) output.value = `${matched.size} / ${nodes.length}`;
}
...
function bindWorkspace(workspace) {
  workspace.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (action === "open-rule-center") setTab("tableManager");
      ...
      if (action === "save-state-prompt" || action === "restore-state-prompt") {
        ...
        if (action === "restore-state-prompt") {
          ...
          const admissionRules = safeText(workspace.querySelector('[data-ma11-state-prompt="admissionRules"]')?.value, 8e3).trim();
          const exclusionRules = safeText(workspace.querySelector('[data-ma11-state-prompt="exclusionRules"]')?.value, 8e3).trim();
          const routingRules = safeText(workspace.querySelector('[data-ma11-state-prompt="routingRules"]')?.value, 8e3).trim();
          const evidenceRules = safeText(workspace.querySelector('[data-ma11-state-prompt="evidenceRules"]')?.value, 8e3).trim();
          const updateRules = safeText(workspace.querySelector('[data-ma11-state-prompt="updateRules"]')?.value, 8e3).trim();
          if (!admissionRules || !exclusionRules || !routingRules || !evidenceRules || !updateRules) throw new Error("\u4E94\u4E2A\u4E8B\u5B9E\u63D0\u53D6\u89C4\u5219\u533A\u90FD\u4E0D\u80FD\u4E3A\u7A7A");
      ...
      if (action === "copy-current-prompt") {
        ...
        if (!prompt) throw new Error("\u5F53\u524D\u6CA1\u6709\u53EF\u590D\u5236\u7684\u63D0\u793A\u8BCD");
  ...
  workspace.addEventListener("change", (event) => {
    ...
    if (target instanceof HTMLSelectElement && target.name === "targetTableKey") {
      ...
      if (form) applyRowEditorTable(form, target.value);
    }
    ...
    if (target.dataset.ma11EntryLimit) {
      ...
      const value2 = Math.min(2e4, Math.max(200, Math.round(Number(target.value) || 1200)));
      getSettings().contentLimits.tables[key] = value2;
      target.value = String(value2);
      saveSettings();
    }
    if (target.dataset.ma11SummaryLimit) {
      const key = target.dataset.ma11SummaryLimit;
      const minimum = key === "largeSummary" ? 300 : 200;
      const maximum = key === "largeSummary" ? 2e4 : 1e4;
      const value2 = Math.min(maximum, Math.max(minimum, Math.round(Number(target.value) || (key === "largeSummary" ? 1800 : 700))));
      getSettings().contentLimits[key] = value2;
      target.value = String(value2);
      saveSettings();
    }
    ...
    if (target.dataset.ma11PromptInspector !== void 0) {
  ...
  workspace.addEventListener("input", (event) => {
    const target = event.target;
    if (target.dataset.ma11TableSearch !== void 0) {
      applyTableSearch(workspace, target.value);
      return;
    }
    if (target.dataset.ma11EventProfileSearchInput !== void 0) {
      applyEventProfileSearch(workspace, target.value);
      return;
    }
    if (target.dataset.ma11GraphSearch !== void 0) {
...
function installMessagePanelHandlers() {
  ...
  const click = (event) => {
    ...
    if (target.closest('[data-ma-action="toggle-inline"]')) {
      if (expandedPanelIndexes.has(index)) expandedPanelIndexes.delete(index);
      else expandedPanelIndexes.add(index);
