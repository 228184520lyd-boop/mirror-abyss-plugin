...
`2.0.0-alpha.5-realtest.1` 的安装候选生成 `esm.sh` 与 `jsDelivr +esm` 两套浏览器 ESM 运行映射，并按固定顺序回退。`dist/DEPENDENCIES.json` 记录直接依赖版本与实际地址。该分发方式不改变各依赖原许可证，也不表示项目复制或改写了这些库的实现。
