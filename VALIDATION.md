...
- 500 字自然事实模块解析通过，确认状态解析器不再执行 220/320 字硬拒绝。
- 默认状态提示词包含“最精炼、最简洁”，不包含“一到两句”字数要求。
- 含角色与物品的成功快照产生 2 个源行和 2 个世界书文档，发布层不再因缺少事件关联而删除。
- 服务端传输模拟完成写入、回读和托管键验收；缺少、正文不一致及残留条目均会失败。
- 所有运行 JavaScript 通过语法检查，所有静态相对导入均可解析。

真实 SillyTavern 环境仍需验证具体部署的鉴权请求头、世界书编辑器刷新和强制同步按钮交互。

## 1.3.16 世界书字段关联专项

- 对象变化仅存在于 `currentFacts` 时，角色世界书条目仍被选中并发布。
- 事件容器对象名仅存在于 `relatedObjects` 时，发布关联判断仍能识别对象。
- 事件无 `MA_CORE`、正文只等于事件名时，不再只发布事件容器或形成空世界书。
- 已结束事件移除后，对象条目仍可独立发布。

# Mirror Abyss 1.3.15 验收记录

流水线：`ma-pipeline-78`

## 1.3.15 事实唯一归属专项

- 对安装包内全部运行时 JavaScript 执行 `node --check`，通过。
- 无 `<MA_CORE>` 的事件可以同时写入“莉娅失去短剑”和“凯厄斯获得短剑”，并由插件生成只含名称与状态的事件容器。
- 关闭事件的本地容器可以进入既有 `retire` 结算指令，事件沉降链未断。
- 对象事实行重复对象主语会被拒绝；同一对象跨语义层返回完全相同事实会被拒绝。
- 不同对象拥有相同状态谓语仍可分别记录，不做错误合并。
- `<MA_TURN>无新增事实</MA_TURN>` 不生成对象事实。
- 未编辑的 1.3.14 标准状态提示词自动迁移；玩家自定义提示词不覆盖。
- 本安装包不包含源仓库测试脚本，因此未声明不存在的 `verify:1.3.15` 命令或完整源测试套件结果。


## 1.3.14 自然事实与结算专项

- `npm run test:rc77:natural`：覆盖固定自然文本事件模块、多个物品分别分发、唯一核心事实、模块长度和重复保护、旧键值协议拒绝、事件与临时 NPC 的 `active → settling → 物理删除`。
- 同一专项覆盖明确事件改变人工／基础保护对象定义时稳定 ID 不变、旧基础事实进入变化历史、完全锁定保护不变，以及临时外观不能越权改写基础定义。
- 小总结待处理窗口与完整来源分离；成功总结只清空已消费窗口，后续语义更新必须携带新消息来源才会重新达到阈值。
- 小总结和大总结只按 `MA_MEMORY` 给匹配对象写入低精度结果；没有对象分发就不删除精确事实。完成长期分发后，旧近期内容和处理用 event/fact 引用退出。
- 世界书发布检查确认不包含内部事实 ID、事件 ID、维护权限、通用 active 状态和与标题完全相同的正文；递归和向量由统一 ST 接管设置决定。
- UI 静态检查确认手机端对象卡片存在，图谱不再使用 `Math.sin/Math.cos` 圆环坐标，画布为 `1000 × 760` 关系簇并使用移动端约 `60vh` 主区域，节点详情顺序位于图谱之后。
- `npm run verify:1.3.14` 用于该版聚焦验收。旧 `verify` 包含大量已废弃 `<MA_CHANGE>` 夹具，未作为本版通过依据；本记录不虚构旧聚合链退出码。

## 固定语义协议专项

- `rc76-semantic-state-protocol`：覆盖“当前状态 → currentStates”等内部映射、未知语义层拒绝、旧 `<MA_ROW>/field=` 协议拒绝、自定义字段显示名映射，以及模型上下文不暴露内部字段键。
- `rc20 / rc29 / rc31 / rc51 / rc52 / rc57 / rc58 / rc69 / rc70 / rc74`：动态表格、可编辑表头、提示长度、固定文本边界、稀疏修订、对象目录、无观点事实、504 降级和生命周期单一权威均通过。
- `rc28:stage6 / rc59:player-runtime`：多轮离线玩家链通过，稳定 ID 继承、总结版本、世界书幂等、聊天切换迟到结果拦截、重复对象 0、幽灵任务 0。
- 旧 `<MA_FACT>`、`<MA_ROW>`、`field=` 与生命周期模型指令只保留明确拒绝测试，不存在生产兼容入口。
- 所有 `verify` 组成命令、`typecheck`、`build` 和语法检查均通过。聚合命令因环境 300 秒上限在 `rc56` 后终止；中断前无失败，中断后的组成命令已独立执行通过，不虚构聚合退出码 0。

## 历史工作流专项

- `rc75-workflow-state`：覆盖 clean、invalid、rebuilding、failed、partial，以及起点选择、暂停错误、最新正文解锁和页面中断。
- `rc32 / rc33 / rc38 / rc42 / rc56 / rc59 / rc66`：历史事务、恢复发布、自动任务竞争、主账本、失败续跑、聊天切换 ABA 和审核检查点均通过。
- `rc72:lorebook`：历史阻断期间世界书发布权限保持原行为。
- 全部 verify 组成命令分段执行通过；单次聚合命令因外部总时限中断，不声明聚合退出码 0。
- `typecheck`、`build`、`node --check index.js`：通过。

## 单一权威专项

- `rc74-single-authority-kernel`：模型旧生命周期字段无写权限；状态机是唯一生产编排入口；`snapshot.ts` 不导出生命周期写操作；世界书不执行对象修复或创建。
- `rc72`：禁止 active 直接删除、覆盖前保留、同稳定 ID 恢复、总结先写覆盖再结算、旧墓碑保守迁移、待结算世界书完全停用。
- `rc68–rc71`：事实增量结算、时空单例、慢模型容错、事件画像与关系图谱融合均通过。
- `rc19`：世界书仍执行纯旁观者的只读过滤；该过滤不会写回快照或成为第二套状态机。
- `typecheck`、`build`、`node --check index.js`：通过。

## 回归说明

完整 `verify` 首轮发现旧 rc28 测试夹具把当前存档错误构造成旧 schema；修正夹具后该测试通过。再次聚合运行受执行环境总时限影响，在耗时异常矩阵区段被外部终止。停止前全部测试通过，`rc59:abnormal` 及其后直到 rc74、类型、构建和语法检查均随后独立执行通过。因此全部组成命令均通过，但不把受外部时限终止的聚合命令声明为退出码 0。
