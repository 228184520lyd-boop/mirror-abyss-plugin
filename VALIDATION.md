...
- 当前聊天托管条目按规范化完整名称归并；
- 同名候选正文或 logicalKey 不同也不会新建第三条；
- 已有当前 managedKey 时优先保留该 UID，否则保留最小 UID；
- 保留 UID 在后续正文更新中持续复用；
- “按对象清理并重新发布”实际删除现代 key 的同名副本；
- 其他聊天、人工条目与不同完整名称不被删除；
- desired 侧名称不唯一时禁止名称误合并；
- logicalKey 多候选且名称不匹配时仍保持原歧义保护。

实机验收：安装刷新后打开出现重复项的聊天，点击“按对象清理并重新发布”。同名镜渊托管条目应只保留一条；再次同步、刷新和再次点击按钮，UID 与条目数量均不增长。
